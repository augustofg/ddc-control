...
fn main() {
    ...
    let (feature, value, write) = if let Some(feature_arg) = matches.subcommand_matches("get-vcp") {
        (
        ...
        panic!("Invalid arguments! Check the argument parsing logic.")
    };

    let mut displays = Display::enumerate();

    let display = if monitor <= displays.len() {
